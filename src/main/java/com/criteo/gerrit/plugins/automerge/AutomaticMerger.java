...
public class AutomaticMerger implements EventListener, LifecycleListener {
  ...
  private void onRefEvent(final RefEvent event) {
    ...
    try {
      RefUpdateAttribute refUpdate = ((RefUpdatedEvent) event).refUpdate.get();
      String commitId = refUpdate.newRev;

      api.changes().query("commit:" + commitId).get()
        .forEach(change -> {
          try {
            api.changes().query("branch:" + change.branch + " project:" +
              change.project + " is:submittable").get()
              .forEach(submittable -> {
                try {
                  atomicityHelper.mergeReview(submittable.project,
                    submittable._number);
                } catch (RestApiException | OrmException | IOException e) {
                  log.error("Cannot autoSubmit change " + change.changeId, e);
                }
              });
          } catch (RestApiException e) {
            log.error("Cannot query submittable changes for branch: " +
              change.branch + " project: " + change.project);
          }
        });

    } catch (RestApiException e) {
      log.error(
        "An exception occured while querying change for ref: " + refName, e);
    }
  }
